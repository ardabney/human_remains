...
for(b in 1:8) {
  ...
  for(j in f_sizes){
    ...
    for(i in (1:935)[-select]){
      pmi_lda <- lda(Estimated_PMI ~ ., data = train_set[,c(select,i)], prior = c(1,1,1,1)/4)
...
#Throws Error: Error in lda.default(x, grouping, ...) : 
# variable 6 (etc.) appears to be constant within groups
